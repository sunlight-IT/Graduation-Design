...
static TARGET_CONDI target_conditon = {0, 20, 180, 255, 150, 255, 60, 255, 40, 40, 100, 100};
uint16_t            picture_data[PIC_W][PIC_H];
...
static uint16_t color_match_bin(hsl_t* hsl);
...
void image_display_bin(void) {
  ...
  hsl_t    hsl_tmp;
  uint16_t tmp_data = 0;
  lcd_set_windows(0, 0, 0 + PIC_W - 1, 0 + PIC_H - 1);
  for (i = 0; i < PIC_W; i++) {
    for (j = 0; j < PIC_H; j++) {
      read_color(j, i, &rgb_tmp);
