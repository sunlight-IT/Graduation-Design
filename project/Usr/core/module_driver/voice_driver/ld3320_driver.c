...
static const voidce_cmd_code_t voice_cmd_data[] = {
    CMD_DATA_CFG("xiao jie", k_code_cmd, "收到"),  //
    // CMD_DATA_CFG("dai ma ce shi", k_code_dmcs, "代码测试成功"),  //
    // CMD_DATA_CFG("ce shi wan bi", k_code_cswb, "测试完毕"),      //

    // CMD_DATA_CFG("bei jing", k_code_1, "北京"),   //
    // CMD_DATA_CFG("shang hai", k_code_2, "上海"),  //
    // CMD_DATA_CFG("kai deng", k_code_3, "开灯"),   //
    // CMD_DATA_CFG("guan deng", k_code_4, "关灯"),  //
    CMD_DATA_CFG("ni hao", k_code_dmcs, "代码测试成功"),  //
    CMD_DATA_CFG("ni hao", k_code_cswb, "测试完毕"),      //

    CMD_DATA_CFG("ni hao", k_code_1, "北京"),  //
    CMD_DATA_CFG("ni hao", k_code_2, "上海"),  //
    CMD_DATA_CFG("ni hao", k_code_3, "开灯"),  //
    CMD_DATA_CFG("ni hao", k_code_4, "关灯"),  //
};
...
bool asr_recognize(void) {
  for (size_t i = 0; i < 5; i++)  //	防止由于硬件原因导致LD3320芯片工作不正常，所以一共尝试5次启动ASR识别流程
  {
    ld3320_init_asr();
    HAL_Delay(5);
    ...
    return true;  //	ASR流程启动成功，退出当前for循环。开始等待LD3320送出的中断信号
  }
  return false;
}
...
void process_int(void) {
  ...
  if ((reg_val & 0x10) && ld3320_read_reg(0xb2) == 0x21 && ld3320_read_reg(0xbf) == 0x35) /*识别成功*/
  {
  ...
  ld3320_write_reg(0x1C, 0); /*写0:ADC不可用*/
  ld3320_write_reg(0x29, 0);
  ld3320_write_reg(0x02, 0);
  ld3320_write_reg(0x2B, 0);
  ld3320_write_reg(0xBA, 0);
  ld3320_write_reg(0xBC, 0);
  ld3320_write_reg(0x08, 1); /*清除FIFO_DATA*/
  ld3320_write_reg(0x08, 0); /*清除FIFO_DATA后 再次写0*/
