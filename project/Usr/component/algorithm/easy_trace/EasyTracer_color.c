...
/*
        ①数据类型备注（STM32F4）：
                ...
                typedef uint16_t     int uint16_t; u16 (int可省)为16位数据类型，占两个字节，范围为0~65535
...
// 读取RBG格式颜色，唯一需要移植的函数
extern uint16_t get_picture_data(uint16_t x, uint16_t y);  // 读某点颜色
...
static void ReadColor_bina(unsigned int x, unsigned int y) {
  uint16_t C16;  // 即Color16，R5+G6+B5=16
 ...
 *返回参数：成功返回1，失败返回0
 ****************************************************************************/
static int SearchCentre_binazition(unsigned int *x, unsigned int *y, const TARGET_CONDI *Condition, const SEARCH_AREA *Area) {
  unsigned int SpaceX, SpaceY, i, j, k, FailCount = 0;
  uint16_t     condition_x = 0, condition_y = 0;
  uint16_t     C16;
  COLOR_RGB    Rgb;
  COLOR_HSL    Hsl;
  ...
  for (i = Area->Y_Start; i < Area->Y_End; i += SpaceY)  // i等于纵轴起始点，i小于纵轴结束点，i每次自加目标最小高度的1/3
  {
    for (j = Area->X_Start; j < Area->X_End; j += SpaceX)  // j等于横轴起始点,j小于横轴结束点，j每次自加目标最小宽度1/3
    {
      ...
      for (k = 0; k < SpaceX + SpaceY; k++)  // k小于目标最小宽度与目标最小高度和
      {
        if (k < SpaceX) {
          condition_x = j + k;
          condition_y = i + SpaceY / 2;
          // 读点j+k,i+SpaceY/2的颜色
        }  // 如果k小于目标最小宽度
        else {  // 否则
          condition_x = j + SpaceX / 2;
          condition_y = i + (k - SpaceX);
          // 读点j+SpaceX/2,i+(k-SpaceX)的颜色
        }
        condition_x = (condition_x >= Area->X_End) ? (Area->X_End - 1) : condition_x;
        condition_y = (condition_x >= Area->Y_End) ? (Area->Y_End - 1) : condition_y;
        C16         = get_picture_data_bina(condition_x, condition_y);
        if (C16 == 65535)  // 进行颜色匹配，如果颜色匹配不成功
          FailCount++;

        if (FailCount > ((SpaceX + SpaceY) >> ALLOW_FAIL_PER))  // 如果失败次数大于容错率
          break;                                                // 结束该长方形区域的搜索循环，移动到下一个区域
      }
      // printf("failpoint = %d  k = %d, spx+spy = %d\n", FailCount, k, SpaceX + SpaceY);
  ...
  for (i = Area->Y_Start; i < Area->Y_End; i += SpaceY)  // i等于纵轴起始点，i小于纵轴结束点，i每次自加目标最小高度的1/3
  {
    for (j = Area->X_Start; j < Area->X_End; j += SpaceX)  // j等于横轴起始点,j小于横轴结束点，j每次自加目标最小宽度1/3
    {
      ...
      for (k = 0; k < SpaceX + SpaceY; k++)  // k小于目标最小宽度与目标最小高度和
      {
        ...
        ReadColor(condition_x, condition_y, &Rgb);              // 读点j+k,i+SpaceY/2的颜色
...
static int Corrode(unsigned int oldx, unsigned int oldy, const TARGET_CONDI *Condition, RESULT *Resu) {
  ...
  Resu->h = Ymax - Ymin;
  // 查看物体宽高度是否符合
  if (((Xmax - Xmin) > (Condition->WIDTH_MIN)) && ((Ymax - Ymin) > (Condition->HIGHT_MIN)) && ((Xmax - Xmin) < (Condition->WIDTH_MAX)) && ((Ymax - Ymin) < (Condition->HIGHT_MAX)))
    return 1;
  else
    return 0;
}

static int Corrode_bina(unsigned int oldx, unsigned int oldy, const TARGET_CONDI *Condition, RESULT *Resu) {
  unsigned int Xmin, Xmax, Ymin, Ymax, i, FailCount = 0;
  COLOR_RGB    Rgb;
  COLOR_HSL    Hsl;
  for (i = oldx; i > IMG_X; i--)         // 左腐蚀，y不变
  {                                      // RGB转换HSL
    if (get_picture_data_bina(i, oldy))  // 进行颜色匹配
      FailCount++;                       // 错误次数自增
    if (FailCount > (((Condition->WIDTH_MIN + Condition->WIDTH_MAX) >> 2) >> ALLOW_FAIL_PER)) break;
  }
  Xmin      = i;  // 更新X轴最小坐标值
  ...
  for (i = oldx; i < IMG_X + IMG_W; i++)  // 右腐蚀，y不变
  {
    if (get_picture_data_bina(i, oldy)) FailCount++;
    if (FailCount > (((Condition->WIDTH_MIN + Condition->WIDTH_MAX) >> 2) >> ALLOW_FAIL_PER)) break;
  }
  Xmax      = i;  // 更新X轴最大坐标值
  FailCount = 0;  // 清空错误次数
  ...
  for (i = oldy; i > IMG_Y; i--)  // 下腐蚀，x不变
  {
    if (get_picture_data_bina(oldx, i)) FailCount++;
    if (FailCount > (((Condition->HIGHT_MIN + Condition->HIGHT_MAX) >> 2) >> ALLOW_FAIL_PER)) break;
  }
  Ymin      = i;  // 更新Y轴最小坐标值
  FailCount = 0;  // 清空错误次数
...
int Trace_bina(const TARGET_CONDI *Condition, RESULT *Resu) {
  ...
  if (flag == 0)  // 如果首次使用或上一次腐蚀失败
  {
    if (SearchCentre_binazition(&x0, &y0, Condition, &Area))  // 搜索腐蚀中心并返回给x0，y0，如果成功搜索到，那么flag置1
      flag = 1;
    else  // 如果还没腐蚀成功，那么把腐蚀区域再次扩大到整个图像范围内进行腐蚀
    {
      Area.X_Start = IMG_X;
      ...
      if (SearchCentre_binazition(&x0, &y0, Condition, &Area))  // 如果整个范围腐蚀成功，那么flag置1
      {
        flag = 1;  // default 0
        return 1;
      } else {
  ...
  for (i = 0; i < ITERATE_NUM; i++)  // 进行腐蚀迭代计算
    Corrode_bina(Result.x, Result.y, Condition, &Result);

  if (Corrode_bina(Result.x, Result.y, Condition, &Result))  // 从腐蚀中心向外腐蚀成功
  {
    x0      = Result.x;
    y0      = Result.y;  // 更新腐蚀中心，以便下次使用
    Resu->x = Result.x;
